...
\documentclass [12pt, proquest] {uwthesis}[2015/03/03]
...
\else % if luatex or xelatex
  ...
  \newcommand{\euro}{€}
   % \setmainfont[Scale=1.12]{Crimson}
   % \def\uwsinglespace{\def\baselinestretch{1.08}\normalfont}
   % \setsansfont{Helvetica}
   % \setmonofont[Mapping=tex-ansi]{Hack}
\fi
...
\ifxetex
  \usepackage[unicode=false, xetex]{hyperref}
\else
...
\section{What is it; why is it spelled that way; and what do really long
section titles look like in the text and in the Table of Contents?}
...
\subsection{Unicode characters and modern fonts}

One of the more antiquated aspects of the \TeX\ system is its
use of pure ASCII character inputs and system specific fonts.
Users of modern word processing systems have long ago left such
things behind. Fortunately, in the \TeX\ world support for Unicode
input (with all of the fancy characters it brings) and access to
all of the True/OpenType fonts on a modern computer are available
using the XeTeX system. \verb%http://xetex.sourceforge.net/%

Once installed, it is a replacement for the {\tt pdflatex} command,
just use {\tt xelatex} instead.
